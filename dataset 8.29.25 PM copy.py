...
class MLSLTDataset(Dataset):
    ...
    def _get_labels(self, json_path):
        ...
        print(f'The json path for getting labels is {json_path}')
    ...
    def _collate_pad(self, batch_samples):
        ...
        print(max_video_len) #to adjust the 500 above
        for video, video_length, transcript, attention_mask in batch_samples:
            # Pad video frames
            print('video shape before padding')
            print(video.shape)
            if video_length < max_video_len:
                ...
                print('video shape after padding')
            print(video.shape)    
